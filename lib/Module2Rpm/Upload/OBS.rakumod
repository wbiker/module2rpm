...
class Module2Rpm::Upload::OBS {
    ...
    method upload-files(Module2Rpm::Package :$package!) {
        ...
        my $url-changes-file = $!api-url ~ "/source/" ~ $!project ~ "/" ~ $package.module-name ~ "/" ~ $package.changes-file-name;
        ...
        my $changes-file-content = $package.changes-file-path.slurp(:close);
        ...
        $!log.info("{$package.module-name}: Upload changes file");
        $!client.put($url-changes-file, body => $changes-file-content);
    }
